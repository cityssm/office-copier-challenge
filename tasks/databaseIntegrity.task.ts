...
function runDatabaseIntegrityTask(): void {
  const purgedCount = purgeCopierCountsByHour()
  const backfilledCount = backfillMissingCopierCountHours()

  debug(
    `Database integrity updated: purged ${purgedCount.toString()} unnecessary records and backfilled ${backfilledCount.toString()} missing hourly records.`
  )
}
